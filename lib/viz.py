...
class BestColorChart(BaseChart):
    """
    Displays the *average number of spells revived* by recoloring a single land
    to each color, on each turn. Only includes columns that actually exist (i.e. used colors).
    """
    def render_spec(self) -> dict:
        plot_width = 600
        plot_height = 300
        ...
        # Instead of assuming all CANONICAL_COLORS, we only transform-fold the columns that exist
        rename_map = {}
        available_colors = []
        for c in CANONICAL_COLORS:
            col_name = f"avg_revived_{c}"
            if col_name in self.df.columns:
                rename_map[col_name] = c
                available_colors.append(c)

        if not available_colors:
            # If no columns exist for revived spells, display "No data" chart
            return alt.Chart(pd.DataFrame({"No Data": []})).mark_text(text="No color data.").to_dict()
        ...
        chart = (
            ...
            .encode(
                ...
                color=alt.Color(
                    ...
                    legend=alt.Legend(title="Mana color")
                )
            )
            .properties(
                width=500,
...
def get_card_color(card_str: str) -> str:
    ...
    used_colors = set(ch for ch in clean if ch in CANONICAL_COLORS)
    if len(used_colors) == 1:
        mapping = {'W': 'grey', 'U': 'blue', 'B': 'black', 'R': 'red', 'G': 'green'}
        return mapping[list(used_colors)[0]]
    else:
        return 'slategray'
...
class SpellDelayChart(BaseChart):
    ...
    Left: cost chart showing the mana cost as a row of circles.
    Right: bubble chart showing distribution of dead turns per spell.
    """
    ...
    def render_spec(self) -> dict:
        ...
        # Determine ordering by expected dead turns descending.
        ...
        bubble = alt.Chart(aggregated).mark_circle().encode(
            x=alt.X(
                ...
                axis=alt.Axis(grid=False, offset=10, labelPadding=5)
            ),
